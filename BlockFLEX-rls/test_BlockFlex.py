...
class TestBlockFLex(unittest.TestCase):
    ...
    def test_net_numvs_barplot_rls(self):
        ...
        sns.barplot(metric_df,y='num_vagrants',x='constellation',hue='network',legend=False)
    ...
    def test_reach_description_rls(self):
        ...
        # metric_df = metric_df.query("duty_ratio == 0.8")
        # metric_df = metric_df.query("duty_ratio == 0.9")
        # metric_df = metric_df.query("duty_ratio == 1")
    ...
    def test_num_redis_plot_rls(self):
        ...
        metric_df = dataloader.get_metric_df(metric='num_rediscovery',  hues=['constellation', 'duty_ratio', 'route'])
        metric_df = metric_df.query("constellation == 'OneWeb'")
        # metric_df = metric_df.query("constellation == 'StarLink'")
    ...
    def test_consumption_plot(self):
        ...
        metric_df['time_consumption']*=1000
    ...
    def test_global_latency_ecdfplot(self):
        ...
        dataloader = DataLoader.load(dir_path="../batch_data/dac_250226/test_jitter_250506/")
    ...
    ''' EXP5
    '''
    def test_local_latency_bar_plot(self):
    ...
    def test_local_lat_lineplot_rls(self):
        ...
        metric_df = dataloader.get_metric_df(metric='latency', hues=['route', 'duty_ratio', 'constellation'],
                                             n_sample=10)
        ...
        # metric_df = metric_df.query("src_dst  in {}".format(['London<->Singapore']))
        metric_df = metric_df.query("src_dst  in {}".format(['Sanya<->Sydney']))
    ...
    def test_local_latency_micro_rls(self):
        dataloader = DataLoader.load(dir_path="./dataset6")

        metric_df = dataloader.get_metric_df(metric='stretch', hues=['constellation', 'duty_ratio', 'route'])
        # metric_df = metric_df.query("duty_ratio == 0.7")
        route_order = ['MLP', 'SDP', 'MHP', 'MDV', 'TNV', 'DABR-$D_{max}=3$', 'DABR-$D_{max}=4$']

        fig, ax = plt.subplots(figsize=[4, 3.2])
        plt.subplots_adjust(left=0.25, right=0.95, bottom=0.27,top=0.96)
        #thp plot
        sns.ecdfplot(metric_df, x="stretch", hue='route', stat='percent', linewidth=2,
                     legend=True,
                     )
        #latency plot
        # sns.barplot(metric_df, x="route", y='stretch',hue='constellation',hue_order=['202','302','402']
        #              ,order = route_order)

        plt.xticks(fontsize=12,rotation=25,ha='right')

        plt.ylabel("Stretch \n (normalized latency)", fontsize=14)
        plt.xlabel('')
        plt.yticks(fontsize=14)
        plt.show()
